...
Candidate evaluation pipeline:
  ...
  3. If XRAY_CHECK_ENABLED: test remaining candidates in parallel via Xray binary,
     dedup by exit IP as well. ipcheck.py also rejects Cloudflare-range exit IPs
     and this server's own IP (see ipcheck.py / cf_ranges.py / config.SERVER_IP).
...
Country codes for each managed slot are recorded in slots_meta.json
(see slots_meta.py) whenever a slot is filled or replaced — this is kept
entirely separate from the panel outbound object, so outbound tags are
never modified for this purpose.
"""
...
HELP_TEXT = (
    ...
    "\n"
    "/locations\n"
    "\u2514 List all managed slots with their source country\n"
    "\n"
    "/checkall\n"
...
BOT_COMMANDS = [
    ...
    BotCommand("locations", "List all managed slots with their source country"),
...
def _record_countries(tag_to_outbound: dict) -> None:
    """Persist each newly-assigned slot's country code (best effort)."""
    for tag, ob in tag_to_outbound.items():
        country = ob.get("_meta", {}).get("country", "")
        if country:
            slots_meta.set_country(tag, country)
...
def _collect_candidates_parallel(
    ...
    `candidates` is a list of (link, country_code) tuples from scraper.py.
    The country code is attached to each parsed outbound's _meta dict so it
    survives through to the caller — merger.py strips _meta before saving,
    so it never reaches the panel; it is only used to populate slots_meta.py.
    ...
    parsed: list[dict] = []
    for link, country in candidates:
        ob = converter.parse_link(link)
        if ob is None:
            continue
        ob.setdefault("_meta", {})["country"] = country
...
async def cmd_fill(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if tag_to_outbound:
        ...
        if err:
            ...
            return
        _record_countries(tag_to_outbound)

    lines = [f"\u2705 /fill {n} done."]
    for tag, ob in tag_to_outbound.items():
        meta = ob.get("_meta", {})
        country = meta.get("country") or "?"
        lines.append(f"  {tag} \u2192 {meta.get('address')}:{meta.get('port')} [{ob.get('protocol')}] ({country})")
...
async def cmd_locations(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    countries = slots_meta.all_countries()

    lines = [f"\U0001f30d {len(managed)} managed slot(s) \u2014 locations:\n"]
    for ob in managed:
        tag = ob.get("tag", "?")
        country = countries.get(tag, "unknown")
